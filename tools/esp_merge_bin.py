...
def get_fw_version(source, target, env):
    ...
    for item in buildFlags.get("CPPDEFINES"):
        if (type(item) is list):
            if (item[0]=="VER_MAJ"): VER_MAJ = item[1]
            if (item[0]=="VER_MIN"): VER_MIN = item[1]
            if (item[0]=="VER_REV"): VER_REV = item[1]
...
def copy_merge_bins(source, target, env):
    version = 'v' + str(VER_MAJ) + '.' + str(VER_MIN) + '.' + str(VER_REV) + '_' + get_firmware_commit_hash()
...
def create_manifest(source, target, env):
    short_version = 'v' + str(VER_MAJ) + '.' + str(VER_MIN) + '.' + str(VER_REV)
