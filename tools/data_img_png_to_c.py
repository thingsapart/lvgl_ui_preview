...
try:
    ...
    env.Execute("$PYTHONEXE -m pip install -vvv pillow")
    from PIL import Image
...
name2const = {
    ...
    "RGB565A8": Converter.FLAG.CF_TRUE_COLOR_565A8,
...
img_dir = os.path.join(project_dir, 'data', 'img')
out_dir = os.path.join(project_dir, 'src', 'img_data')

for filename in os.listdir(img_dir):
    try:
        if os.path.splitext(filename)[1].lower() == '.png':
            conv_one_file(Path(project_dir), Path(os.path.join(img_dir, filename)), "alpha_8", "alpha_8", "C", True, False, Path(out_dir))
