...
def _generate_generic_invoke_fn(signature_category, sig_c_name, function_list, api_info):
    ...
    c_code += f"        int current_c_code_arg_idx = 0; // Renamed from c_code_arg_count to avoid conflict in loop\n"
    c_code += f"        bool success = true; // Changed from transpile_arg_ok for clarity\n"
    c_code += f"        int c_arg_idx_for_unmarshal = 0;\n"
    c_code += f"        if ({str(first_arg_is_target).lower()}) {{ // Python bool to C bool\n"
    # c_code += f"            // First C arg is the target object itself, already provided as target_obj_c_name_transpile\n"
    # c_code += f"            // No JSON arg corresponds to it, but we might need to add it to a list if building call string generically.\n"
    # c_code += f"            // For now, assume target_obj_c_name_transpile is directly used in sprintf.\n"
    ...
    c_code += f"        for (int i = 0; i < num_json_args_to_process; ++i) {{\n"
    c_code += f"            cJSON *json_arg_item = cJSON_GetArrayItem(args_array, i);\n"
    c_code += f"            const char* expected_arg_type = entry->arg_types[c_arg_idx_for_unmarshal];\n"
    c_code += f"            char* arg_c_str = NULL;\n"
    c_code += f"            bool is_nested_call = false;\n"
    c_code += f"            if (cJSON_IsObject(json_arg_item)) {{\n"
    c_code += f"                cJSON *nested_call_name_item = cJSON_GetObjectItemCaseSensitive(json_arg_item, \"call\");\n"
    c_code += f"                cJSON *nested_call_args_item = cJSON_GetObjectItemCaseSensitive(json_arg_item, \"args\");\n"
    c_code += f"                if (nested_call_name_item && cJSON_IsString(nested_call_name_item) && nested_call_args_item && cJSON_IsArray(nested_call_args_item)) {{\n"
    c_code += f"                    is_nested_call = true;\n"
    c_code += f"                    const char* nested_func_name = nested_call_name_item->valuestring;\n"
    c_code += f"                    const invoke_table_entry_t* nested_entry = find_invoke_entry(nested_func_name);\n"
    c_code += f"                    if (!nested_entry) {{\n"
    c_code += f"                        LOG_ERR_JSON(json_arg_item, \"Transpile Error: Nested call function '%s' not found.\", nested_func_name);\n"
    c_code += f"                        success = false; break;\n"
    c_code += f"                    }}\n"
    c_code += f"                    // Pass target_obj_ptr_render_mode as context for resolving @pointers in nested call's args\n"
    c_code += f"                    // Target for the nested call itself is assumed NULL for now (static or contextless method)\n"
    c_code += f"                    if (!nested_entry->invoke(nested_entry, target_obj_ptr_render_mode, NULL, NULL, &arg_c_str, nested_call_args_item)) {{\n"
    c_code += f"                        LOG_ERR_JSON(json_arg_item, \"Transpile Error: Nested call to '%s' failed during transpilation.\", nested_func_name);\n"
    c_code += f"                        success = false; break;\n"
    c_code += f"                    }}\n"
    c_code += f"                    if (!arg_c_str) {{\n"
    c_code += f"                        LOG_ERR_JSON(json_arg_item, \"Transpile Error: Nested call to '%s' did not produce a C variable name (result might be void or error).\", nested_func_name);\n"
    c_code += f"                        success = false; break;\n"
    c_code += f"                    }}\n"
    c_code += f"                }}\n"
    c_code += f"            }}\n"
    c_code += f"            if (!is_nested_call) {{ // Not a nested call, or not a valid one, unmarshal directly to C string\n"
    c_code += f"                if (!unmarshal_value_to_c_string(json_arg_item, expected_arg_type, &arg_c_str, target_obj_ptr_render_mode)) {{\n"
    c_code += f"                    LOG_ERR_JSON(json_arg_item, \"Transpile Error: Failed to convert arg %d ('%s') to C string for %s.\", i, expected_arg_type, entry->name);\n"
    c_code += f"                    success = false; break;\n"
    c_code += f"                }}\n"
    c_code += f"            }}\n"
    c_code += f"            c_code_args[current_c_code_arg_idx++] = arg_c_str;\n"
    c_code += f"            c_arg_idx_for_unmarshal++;\n"
    c_code += f"        }}\n\n"
    c_code += f"        if (!success) {{\n"
    c_code += f"            for (int k = 0; k < current_c_code_arg_idx; ++k) lv_free(c_code_args[k]); // Free already processed args\n"
    c_code += f"            return false;\n"
    c_code += f"        }}\n\n"
    c_code += f"        char call_str[1024]; call_str[0] = '\\0';\n"
    c_code += f"        char all_args_c_str[512]; all_args_c_str[0] = '\\0';\n"
    # Construct argument list string from c_code_args
    c_code += f"        for (int k = 0; k < current_c_code_arg_idx; ++k) {{\n"
    c_code += f"            if (c_code_args[k]) {{ strcat(all_args_c_str, c_code_args[k]); }}\n" # Check for NULL if nested void call was allowed
    c_code += f"            else {{ strcat(all_args_c_str, \"NULL /*arg error*/\"); }}\n" # Should not happen if success is true
    c_code += f"            if (k < current_c_code_arg_idx - 1) strcat(all_args_c_str, \", \");\n"
    c_code += f"        }}\n\n"
    ...
    c_code += "             g_transpile_var_counter++;\n"
    c_code += f"            *result_c_name_transpile = transpile_generate_c_var_name(NULL, entry->ret_type, g_transpile_var_counter);\n"
    c_code += f"            if (!*result_c_name_transpile) {{ LOG_ERR(\"TRANSPILE_MODE: Failed to generate result C name for %s\", entry->name); success = false; }}\n" # Set success to false
    c_code += f"            else {{\n"
    c_code += f"                if (is_method_call) {{\n"
    c_code += f"                    snprintf(call_str, sizeof(call_str), \"    %s %s = %s(%s%s%s);\", entry->ret_type, *result_c_name_transpile, entry->name, effective_target_c_name ? effective_target_c_name : \"NULL_TARGET_ERR\", (current_c_code_arg_idx > 0 ? \", \" : \"\"), all_args_c_str);\n"
    c_code += f"                }} else {{\n"
    ...
    c_code += f"        }} else {{\n"
    c_code += f"            if (is_method_call) {{\n"
    c_code += f"                snprintf(call_str, sizeof(call_str), \"    %s(%s%s%s);\", entry->name, effective_target_c_name ? effective_target_c_name : \"NULL_TARGET_ERR\", (current_c_code_arg_idx > 0 ? \", \" : \"\"), all_args_c_str);\n"
    c_code += f"            }} else {{\n"
    c_code += f"                snprintf(call_str, sizeof(call_str), \"    %s(%s);\", entry->name, all_args_c_str);\n"
    c_code += f"            }}\n"
    c_code += f"        }}\n\n"
    c_code += f"        if (success) {{ transpile_write_line(g_output_c_file, call_str); }}\n\n"
    c_code += f"        for (int k = 0; k < current_c_code_arg_idx; ++k) lv_free(c_code_args[k]);\n"
    c_code += f"        if (!success && result_c_name_transpile && *result_c_name_transpile) {{ lv_free(*result_c_name_transpile); *result_c_name_transpile = NULL; }}\n"
    c_code += f"        return success;\n"
    c_code += f"    }}\n"
