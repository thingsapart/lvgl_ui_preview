...
// Opaque object pointer - points to our internal struct
struct EmulLvglObject;
typedef struct EmulLvglObject* lv_obj_t;

// Coordinates (can be pixel or percentage)
typedef int16_t lv_coord_t;
#define LV_COORD_IS_PCT(v) ((v) <= -1000) // Use a range for percentages unlikely for pixels
...
typedef struct {
    ...
    uint8_t g;
    uint8_t b;
    // No alpha for simplicity in this example, add if needed
} lv_color_t;
...
typedef enum {
    ...
    // Add others like OUT_... if needed
} lv_align_t;
...
#define LV_STATE_FOCUS_KEY  0x0004  // Renamed from FOCUS_KEYED in v8? Check v9 docs if exact name matters
...
// Parts (subset) - Use high bits for parts to allow ORing with states
...
// Font - Opaque pointer. We'll need to map these to names.
 ...
 *
 * @param root_obj The root object of the UI tree to serialize (usually the object created on the screen).
...
// ** Object Deletion/Cleanup (Basic) **
void lv_obj_del(lv_obj_t obj); // Removes object and children from emulation
void lv_obj_clean(lv_obj_t obj); // Removes children from emulation

// ** Screen Access **
lv_obj_t lv_screen_active(void); // lv_scr_act() deprecated, use lv_screen_active() in v9
// lv_obj_t lv_scr_act(void); // Alias for compatibility if needed
...
void lv_obj_add_flag(lv_obj_t obj, uint32_t f); // We'll just store simple flags
void lv_obj_clear_flag(lv_obj_t obj, uint32_t f);
// Define some common flags we might store directly as properties (e.g., "hidden", "clickable")
#define LV_OBJ_FLAG_HIDDEN   (1 << 0) // Example flag mapping
...
// const lv_font_t lv_font_montserrat_14 = (const lv_font_t)0x1; // Just unique pointers
// const lv_font_t lv_font_montserrat_18 = (const lv_font_t)0x2;
