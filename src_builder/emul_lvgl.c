    ...
    remove_from_global_list(obj);
     ...
     if(children_copy) {
         for(size_t i = 0; i < num_children; ++i) {
             ...
             // Check if child pointer still exists in global list
             bool child_exists = false;
             for(size_t j=0; j<g_all_objects_count; ++j) {
                 if(g_all_objects[j] == child_ptr) {
                     child_exists = true;
                     break;
                 }
             }
...
void lv_obj_clean(lv_obj_t *obj) {
     ...
     if (obj->child_count != 0) { EMUL_LOG("WARN: Child count not zero after clean\n"); obj->child_count = 0;}
...
void lv_obj_add_flag(lv_obj_t *obj, uint32_t f) {
    if (!obj) return;
    if (f & LV_OBJ_FLAG_HIDDEN) emul_obj_add_property(obj, "hidden", value_mk_bool(true));
    if (f & LV_OBJ_FLAG_CLICKABLE) emul_obj_add_property(obj, "clickable", value_mk_bool(true));
    // ...
}
void lv_obj_clear_flag(lv_obj_t *obj, uint32_t f) {
    if (!obj) return;
    if (f & LV_OBJ_FLAG_HIDDEN) emul_obj_add_property(obj, "hidden", value_mk_bool(false));
    if (f & LV_OBJ_FLAG_CLICKABLE) emul_obj_add_property(obj, "clickable", value_mk_bool(false));
    // ...
}
...
#define ADD_STYLE(obj_ptr, selector, prop_name_str, value_func) \
    do { \
        ...
        lv_part_t part = selector & 0xFF0000; \
        lv_state_t state = selector & 0x00FFFF; \
...
// --- Helper Value Creators --- (remain the same)
...
lv_coord_t lv_pct(uint8_t v) { if (v > 100) v = 100; return LV_COORD_SET_PCT(v); }
...
char* emul_lvgl_get_json(lv_obj_t *root_obj) { // Takes pointer
    ...
    char* json_string = cJSON_PrintUnformatted(root_container);
...
cJSON* build_json_recursive(lv_obj_t *obj) { // Takes pointer
    ...
    if (obj->prop_count > 0) {
        ...
        for (size_t i = 0; i < obj->prop_count; ++i) {
            ...
            // ... (Switch statement for property value types remains the same) ...
             switch (prop->value.type) {
                case VAL_TYPE_STRING: cJSON_AddStringToObject(props_json, prop->key, prop->value.data.s ? prop->value.data.s : ""); break;
                case VAL_TYPE_INT: cJSON_AddNumberToObject(props_json, prop->key, prop->value.data.i); break;
                case VAL_TYPE_COORD:
                    coord_to_string(prop->value.data.coord, coord_buf);
                    if (strchr(coord_buf, '%')) cJSON_AddStringToObject(props_json, prop->key, coord_buf);
                    else cJSON_AddNumberToObject(props_json, prop->key, prop->value.data.coord);
                    break;
                case VAL_TYPE_BOOL: cJSON_AddBoolToObject(props_json, prop->key, prop->value.data.b); break;
                case VAL_TYPE_ALIGN: cJSON_AddStringToObject(props_json, prop->key, align_to_string(prop->value.data.align)); break;
                default: break;
            }
        }
    }
    ...
    if (obj->style_count > 0) {
        ...
        for (size_t i = 0; i < obj->style_count; ++i) {
            ...
            if (!part_obj) { part_obj = cJSON_CreateObject(); cJSON_AddItemToObject(styles_json, part_str, part_obj); }

            cJSON* state_obj = cJSON_GetObjectItemCaseSensitive(part_obj, state_str);
            if (!state_obj) { state_obj = cJSON_CreateObject(); cJSON_AddItemToObject(part_obj, state_str, state_obj); }

            char buf[12];
            // ... (Switch statement for style value types remains the same) ...
             switch (entry->value.type) {
                case VAL_TYPE_COLOR: color_to_hex_string(entry->value.data.color, buf); cJSON_AddStringToObject(state_obj, entry->prop_name, buf); break;
                case VAL_TYPE_COORD:
                    coord_to_string(entry->value.data.coord, buf);
                     if (strchr(buf, '%')) cJSON_AddStringToObject(state_obj, entry->prop_name, buf);
                     else cJSON_AddNumberToObject(state_obj, entry->prop_name, entry->value.data.coord);
                    break;
                case VAL_TYPE_INT: cJSON_AddNumberToObject(state_obj, entry->prop_name, entry->value.data.i); break;
                case VAL_TYPE_FONT: cJSON_AddStringToObject(state_obj, entry->prop_name, font_ptr_to_name(entry->value.data.font)); break;
                 case VAL_TYPE_TEXTALIGN: cJSON_AddStringToObject(state_obj, entry->prop_name, text_align_to_string(entry->value.data.text_align)); break;
                default: break;
            }
        }
    }
